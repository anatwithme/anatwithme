...
export async function removeAdmin(adminId: string) {
  ...
  if (callError) {
    throw new Error(`ERROR: Removing administrator resulted in the following error - ${callError?.message ?? "Unknown Error"}`);
  }
...
async function loadExistingRoomUsage(
  ...
  const { data, error } = await supabase
    .from("group")
    .select("id, room_id, day_of_week, meet_start_time")
  ...
  return {
    usage: ((data ?? []) as (ExistingScheduledGroupRow & { id: string })[])
      .filter(
        (group) =>
...
export async function updateManualGroup(
  ...
  if (groupError || !existingGroup) {
    console.error("Error loading group for update:", groupError);
  ...
  const alreadyGroupedStudents = students.filter((student) =>
    (student.member_of?.length ?? 0) > 0 &&
    !student.member_of?.some((membership) => membership.group_id === groupId),
  );
  ...
  if (alreadyGroupedStudents.length > 0) {
    return {
      error:
        alreadyGroupedStudents.length === 1
          ? `${alreadyGroupedStudents[0]?.full_name ?? "A selected student"} is already assigned to a different group. Refresh and try again.`
  ...
  if (validation.value.preference === "in_person") {
    ...
    const existingUsageResult = await loadExistingRoomUsage(
      supabase,
      groupId,
    );
    if ("error" in existingUsageResult) {
  ...
  if (currentMemberError) {
    console.error("Error loading current group members:", currentMemberError);
    return { error: "Failed to update group members." };
  }

  const currentMemberIds = ((currentMembers ?? []) as { user_id: string }[]).map(
    (member) => member.user_id,
  );
  const currentMemberSet = new Set(currentMemberIds);
  const newMemberSet = new Set(selectedStudentIds);

  const memberIdsToRemove = currentMemberIds.filter(
    (id) => !newMemberSet.has(id),
  );
  const memberIdsToAdd = selectedStudentIds.filter(
  ...
  const { error: updateError } = await supabase
    ...
    .update({
      ...
      meet_start_time: validation.value.meetStartTime,
      meet_end_time: validation.value.meetEndTime,
      room_id: validation.value.preference === "in_person" ? manualRoomId : null,
      room_overbooked:
        validation.value.preference === "in_person" ? manualRoomOverbooked : false,
    })
    .eq("id", groupId);
  ...
  if (memberIdsToRemove.length > 0) {
    ...
    if (removeError) {
      console.error("Error removing old group members:", removeError);
      return { error: "Failed to update group members" };
    }
  }
  ...
  if (memberIdsToAdd.length > 0) {
    ...
    if (addError) {
      console.error("Error adding new group members:", addError);
      return { error: "Failed to update group members" };
    }
  }
